...
async function listNewOrders(req, res) {
    try {
      ...
      `,
        ...
        return res.json({
            success: true,
            data: { page, limit, total: countRs.rows[0].total, orders },
            message: null,
        });
    } catch (e) {
        ...
        return res.json({
            ...
            data: {
                ...
                items: itemsRs.rows.map(x => ({
                    ...
                    line_total: Number(x.qty) * Number(x.unit_price),
                })),
            },
            message: null,
        });
...
async function acceptNewOrder(req, res) {
    ...
    try {
        ...
        // Chỉ accept nếu đang pending (tránh 2 người bấm cùng lúc)
      ...
      `,
        ...
        if (!upRs.rows.length) {
            ...
            return res.status(409).json({
                success: false,
                data: null,
                message: "Đơn không còn ở trạng thái chờ xử lý hoặc không thuộc bếp trung tâm của bạn",
                error_code: "ORDER_STATUS_CONFLICT",
            });
        }

        await client.query("COMMIT");
        return res.json({ success: true, data: upRs.rows[0], message: "Đã chấp nhận đơn" });
    } catch (e) {
        await client.query("ROLLBACK");
...
async function rejectNewOrder(req, res) {
    ...
    try {
        ...
        // DB không có cột lưu reason -> chỉ validate + log
        ...
        console.warn(
            `[NEW_ORDER_REJECT] kitchen_user=${req.user.user_id} ck=${centralKitchenId} orderId=${orderId} reason="${String(reason).trim()}"`
        );
      ...
      `,
        ...
        await client.query("COMMIT");
        return res.json({ success: true, data: upRs.rows[0], message: "Đã từ chối đơn" });
    } catch (e) {
        await client.query("ROLLBACK");
