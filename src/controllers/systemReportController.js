...
async function systemReport(req, res) {
    try {
        ...
        const users = usersRs.rows[0] || {
            total_users: 0,
            active_users: 0,
            inactive_users: 0
        };

        const stores = storeRs.rows[0] || {
            total_franchise_stores: 0,
            active_franchise_stores: 0,
            inactive_franchise_stores: 0
        };

        const finance = financeRs.rows[0] || {
            total_order_value: 0,
            paid_amount: 0,
            unpaid_amount: 0
        };
        ...
        const collection_rate =
            Number(finance.total_order_value) > 0
                ? Number(
                    (
                        (Number(finance.paid_amount) / Number(finance.total_order_value)) *
                        100
                    ).toFixed(2)
                )
                : 0;
        ...
        return res.json({
            ...
            data: {
                summary_cards: {
                    ...
                    franchise_stores: {
                        active: stores.active_franchise_stores,
                        total: stores.total_franchise_stores
                    },
                    total_orders: totalOrdersRs.rows[0]?.total_orders || 0,
                    total_stock: totalStockRs.rows[0]?.total_stock || 0
                },
                financial: {
